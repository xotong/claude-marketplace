#!/usr/bin/env sh
# =============================================================================
# Scanner      : GitLab SAST (Semgrep analyzer)
...
cd "${CI_PROJECT_DIR}"
mkdir -p "${RESULTS}"
rm -f "${REPORT}" "${RESULTS}/gl-sast-report.json"
...
if [ "${SAST_EXCLUDED_PATHS+x}" = "x" ]; then
  export SAST_EXCLUDED_PATHS
fi

# =============================================================================
# SCAN — mirrors the GitLab CI/CD Catalog component script.
# Component image: "$image_prefix/semgrep:$image_tag$image_suffix"
...
if [ -f "${REPORT}" ]; then
  cp "${REPORT}" "${RESULTS}/gl-sast-report.json"
elif [ ! -f "${RESULTS}/gl-sast-report.json" ]; then
  echo "ERROR: gl-sast-report.json was not produced" >&2
  exit 1
fi
